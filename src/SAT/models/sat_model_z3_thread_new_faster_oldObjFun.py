...
def _sat_worker(queue, n, timeout, optimize, encoding):
    ...
    if optimize:
        ...
        objective_values = list(range(min_obj, max_obj + 1, 4))  # Only values multiple of 4
        ...
        # Binary search over possible even objective INDEXES, not on the values (for simplicity)
        while start < end:
    ...
    else: #if we don't want to do the optimization we just do the decision problem
        decision_problem_feasible()
