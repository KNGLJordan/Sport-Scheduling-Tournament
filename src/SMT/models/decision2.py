    ...
    for team in range(n):
        for w in range(weeks):
            s.add(Sum([If(M[(t1, t2)] == w, 1, 0)
                    for (t1, t2) in matches if team in (t1, t2)]) == 2) #2 instead of 1 because in the matrix there will be both (i,j) and (j,i)

    # 2. Every week have exactly periods match
    for w in range(weeks):
        s.add(Sum([If(M[(t1, t2)] == w, 1, 0) for (t1, t2) in matches]) == 2 * periods)

    # 3. Every week and period have exactly one match
    for w in range(weeks):
        for p in range(periods):
            s.add(Sum([If(And(M[(t1, t2)] == w, P[(t1, t2)] == p), 1, 0)
                    for (t1, t2) in matches]) == 2)

    # 4. Every team can have a match in the same period at most two times
    for team in range(n):
        for p in range(periods):
            s.add(Sum([If(P[(t1, t2)] == p, 1, 0)
                    for (t1, t2) in matches if team in (t1, t2)]) <= 4)
